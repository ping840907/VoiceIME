...
- **雙模型 ASR 架構自由切換**：
  - **X-ASR (Zipformer2 Streaming，約 138MB)**：輕量即時串流模型，邊說邊出字，超低延遲反饋；原生支援台灣繁體中文、國台語及中英夾雜。
  - **Qwen3-ASR (0.6B int8，約 878MB)**：高精確度辨識模型，支援繁體中文輸出、可選中文全形標點符號過濾，並支援自訂停頓偵測秒數（VAD 0.5s - 2.5s）。
...
| 引擎 | 模型大小 | 特點 | 適用情境 |
|---|:---:|---|---|
| **X-ASR** | 約 138MB | Zipformer2 串流推論，邊講邊出字，超低延遲，原生繁中 | 日常快速即時傳送訊息、短句輸入 |
| **Qwen3-ASR** | 約 878MB | Alibaba Qwen3-ASR 0.6B 量化版，辨識率高，支援 VAD 斷句與標點過濾 | 長篇文字輸入、高精準度需求、會議紀錄 |
